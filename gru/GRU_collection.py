...
class BasicWeatherGRU(nn.Module):
    ...
    def generate(self, features, max_length=100, token_mappings=None):
        """
        Generate text from features using sampling for better diversity.
        
        Args:
            features (torch.Tensor): Input features [batch_size, seq_len, feature_dim]
            max_length (int): Maximum length of generated sequence
            token_mappings (dict, optional): Optional token mappings for vocabulary conversion
...
class AttentionWeatherGRU(nn.Module):
    ...
    def generate(self, features, max_length=100, temperature=0.7):
        """
        Generate text from weather features.
        ...
        Returns:
            torch.Tensor: Generated token indices [batch_size, seq_len]
        """
        self.eval()
        with torch.no_grad():
            batch_size = features.size(0)
            ...
            # Process features
            temp_features = features[:, :, 0].unsqueeze(-1)
            humidity_features = features[:, :, 1].unsqueeze(-1)
            cloud_features = features[:, :, 2].unsqueeze(-1)
            time_features = features[:, :, -5:-3]
            ...
            # Start with token ID 0 (typically CLS/start token)
            decoder_input = torch.zeros(batch_size, 1, dtype=torch.long, device=features.device)
            decoder_hidden = encoder_hidden
            ...
            for t in range(max_length):
                ...
                keys = self.attention_projection(encoder_outputs)
                attn_scores = torch.bmm(query, keys.transpose(1, 2)) / math.sqrt(self.hidden_size)
                ...
                probs = F.softmax(scaled_prediction, dim=1)
                next_token = torch.multinomial(probs, 1)
                
                generated_tokens.append(next_token)
                decoder_input = next_token
                
                # Optional: Check for end token (e.g., token_id 102 for SEP/end)
                # if (next_token == end_token_id).any():
                #     break
