...
class TestWeatherDataset(unittest.TestCase):
    def test_correct_path_results(self):
        dset = WeatherDataset("./tests/test_weather_dataset", Split.EVAL, TransformationPipeline([]))
    ...
    def test_correct_path_results_with_n_samples(self):
        dset = WeatherDataset("./tests/test_weather_dataset", Split.EVAL, TransformationPipeline([]), n_samples=2)

        self.assertEqual("./tests/test_weather_dataset/dset_eval_2.json", dset._full_path)
    ...
    def test_correct_number_of_files_if_cached(self):
        dset = WeatherDataset("./tests/test_weather_dataset", Split.EVAL, TransformationPipeline([]), cached=True)
    ...
    def test_correct_dict_is_returned(self):
        ...
        self.assertEqual(expected_keys, list(dset.__getitem__(0).keys()))
